...
def daemon(
    ...
    # verify that various files can be written
    if pidfile: open(pidfile, 'a')
    if logfile: open(logfile, 'a')
    if errorfile: open(errorfile, 'a')
    ...
    def daemonize(function):
        ...
        try:
            if logfile is not None:
                print>>file(logfile, 'a'), 'started.'
            try:
                function()
            except Exception:
                if errorfile is not None:
                    from traceback import print_exc
                    print_exc(file, file = file(errorfile, 'a'))
        finally:
            if logfile is not None:
                print>>file(logfile, 'a'), 'stopped.'
            if pidfile is not None:
                os.unlink(pidfile)
